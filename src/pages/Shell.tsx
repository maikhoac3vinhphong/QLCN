...
type TabId = 'home' | 'record' | 'board' | 'approve'

const TAB_LABEL: Record<TabId, string> = {
  home: 'Trang chủ', record: 'Ghi nhận', board: 'Thi đua', approve: 'Chờ duyệt'
}

function tabsFor(role: Profile['role']): TabId[] {
  switch (role) {
    case 'gvcn': return ['home', 'record', 'board', 'approve']
    case 'totruong': return ['record', 'board', 'home']
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      ...
      <main style={{ flex: 1, maxWidth: 680, width: '100%', margin: '0 auto', padding: '14px 14px 0' }}>
        ...
        {tab === 'approve' && <Approvals profile={profile} classId={classId} />}
  ...
  return (
    <div className="card" style={{ padding: 22 }}>
      ...
      <p style={{ color: 'var(--muted)', marginTop: 16, fontSize: 14, lineHeight: 1.6 }}>
        Điểm danh, bản tin phụ huynh, sơ đồ lớp và các tiện ích khác sẽ được thêm ở các bản kế tiếp.
      </p>
...
function Icon({ id, active }: { id: TabId; active: boolean }) {
  ...
  if (id === 'approve') return <svg viewBox="0 0 24 24" {...s}><path d="M9 12l2 2 4-4" /><circle cx="12" cy="12" r="9" /></svg>
