...
const ROLE_LABEL: Record<Profile['role'], string> = { gvcn: 'Giáo viên chủ nhiệm', totruong: 'Tổ trưởng', hs: 'Học sinh', phhs: 'Phụ huynh' }
...
export default function Shell({ profile, classId, isTreasurer, onSignedOut }: { profile: Profile; classId: string; isTreasurer: boolean; onSignedOut: () => void }) {
  ...
  const page = (
    <>
      ...
      {tab === 'home' && <Home profile={profile} onSignedOut={onSignedOut} desktop={desktop} />}
    </>
  )
  ...
  if (desktop) {
    return (
      <div style={{ display: 'flex', minHeight: '100dvh' }}>
        <aside style={{ width: 244, flexShrink: 0, borderRight: '1px solid var(--line)', background: 'var(--surface)', position: 'sticky', top: 0, height: '100dvh', display: 'flex', flexDirection: 'column' }}>
          <div style={{ padding: '20px 20px 14px' }}>
            <div style={{ fontWeight: 700, fontSize: 22, color: 'var(--primary)', letterSpacing: 1 }}>QLCN</div>
            <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 2 }}>Quản lý chủ nhiệm</div>
          </div>
          <nav style={{ padding: '4px 12px', display: 'grid', gap: 4 }}>
            {tabs.map((t) => (
              <button key={t} onClick={() => setTab(t)} style={{
                display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 10, border: 'none',
                background: tab === t ? 'var(--primary-100)' : 'transparent', color: tab === t ? 'var(--primary)' : 'var(--ink)',
                fontWeight: tab === t ? 600 : 500, fontSize: 15, textAlign: 'left', cursor: 'pointer'
              }}>
                <Icon id={t} active={tab === t} /> {TAB_LABEL[t]}
              </button>
            ))}
          </nav>
          <div style={{ marginTop: 'auto', padding: 16, borderTop: '1px solid var(--line)' }}>
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)', marginBottom: 10 }}>{ROLE_LABEL[profile.role]}</div>
            <button className="btn" style={{ width: '100%', minHeight: 38 }} onClick={async () => { await signOut(); onSignedOut() }}>Đăng xuất</button>
          </div>
        </aside>

        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 26px', borderBottom: '1px solid var(--line)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 10 }}>
            <span style={{ fontWeight: 600, fontSize: 18 }}>{TAB_LABEL[tab]}</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}><NotificationsBell /><NetBadge /></div>
          </header>
          <main style={{ flex: 1, width: '100%', maxWidth: 920, margin: '0 auto', padding: '24px 26px' }}>{page}</main>
          <footer style={{ padding: '14px 26px', borderTop: '1px solid var(--line)', color: 'var(--muted)', fontSize: 13, textAlign: 'center' }}>
            QLCN — Quản lý nề nếp &amp; thi đua lớp · Dùng menu bên trái để chuyển mục.
          </footer>
        </div>
      </div>
    )
  ...
  return (
    <div style={{ minHeight: '100dvh', display: 'flex', flexDirection: 'column' }}>
      <header style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '1px solid var(--line)', background: 'var(--surface)', position: 'sticky', top: 0, zIndex: 10 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}><NotificationsBell /><NetBadge /></div>
      </header>
      <main style={{ flex: 1, maxWidth: 680, width: '100%', margin: '0 auto', padding: '14px 14px 0' }}>{page}</main>
      <nav style={{ position: 'sticky', bottom: 0, display: 'flex', borderTop: '1px solid var(--line)', background: 'var(--surface)', zIndex: 5 }}>
        {tabs.map((t) => (
          <button key={t} onClick={() => setTab(t)} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '8px 0', background: 'transparent', border: 'none', color: tab === t ? 'var(--primary)' : 'var(--muted)', fontSize: 11, fontWeight: 500 }}>
            <Icon id={t} active={tab === t} /> {TAB_LABEL[t]}
          </button>
        ))}
      </nav>
...
function Home({ profile, onSignedOut, desktop }: { profile: Profile; onSignedOut: () => void; desktop: boolean }) {
  return (
    <div className="card" style={{ padding: 22 }}>
      ...
      <p style={{ color: 'var(--muted)', marginTop: 16, fontSize: 14, lineHeight: 1.6 }}>
        {profile.role === 'gvcn' ? 'Vào Tiện ích để thêm học sinh, chia tổ, thu chi quỹ, gửi thông báo, xếp sơ đồ lớp và hơn thế.' : 'Chúc em học tập tiến bộ mỗi ngày.'}
      </p>
      {!desktop && <button className="btn" style={{ marginTop: 8 }} onClick={async () => { await signOut(); onSignedOut() }}>Đăng xuất</button>}
      <PushToggle />
    </div>
  )
}
